...
// @ts-ignore: Metro bundler necesita que no tenga extensión
...
export async function loadAccounts(): Promise<TotpMeta[]> {
  const raw = await (AsyncStorage as any).getItem(LIST_KEY);
  if (!raw) return [];
  try { return JSON.parse(raw) as TotpMeta[]; } catch { return []; }
}

export async function saveAccounts(accounts: TotpMeta[]) {
  await (AsyncStorage as any).setItem(LIST_KEY, JSON.stringify(accounts));
}
